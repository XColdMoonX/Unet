...
def dilation(Input_folder, output_folder):
    ...
    kernel_size = config.getint("Dilation", "kernel_size")
    erode_iterations = config.getint("Dilation", "erode_iterations")
    ...
    for filename in os.listdir(Input_folder):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            if os.path.exists(os.path.join(output_folder, filename)):
                print(
                    f"File {filename} already exists in output directory. Skipping..."
                )
                continue
