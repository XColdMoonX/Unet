...
def all_predict(PredictPath, ModelPath, Input_folder, output_folder, OutputExt=".png"):
    ...
    for file in tqdm(input_files, desc="Processing files"):
        print(f"\nProcessing file: {file}")  # 顯示目前正在處理的檔案
        InputPath = os.path.join(Input_folder, file)  # 完整的輸入檔案路徑
        OutputFileName = (
            os.path.splitext(file)[0] + OutputExt
        )  # 將輸出檔案的副檔名改為.png
        ...
        if os.path.exists(OutputPath):
            print(
                f"File {OutputFileName} already exists in output directory. Skipping..."
            )
            continue

        # 執行 "python predict.py -m ModelPath -i InputPath -o OutputPath" 指令
        subprocess.run(
            ["python", PredictPath, "-m", ModelPath, "-i", InputPath, "-o", OutputPath],
            check=True,
        )
